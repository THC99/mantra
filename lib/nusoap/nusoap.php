...
/*
Modified by: Didi Sukyadi, 2016/10/15
- $SCRIPT_NAME = str_replace("?".$QUERY_STRING,"",$REQUEST_URI);
- $PHP_SELF = str_replace("?".$QUERY_STRING,"",$REQUEST_URI);
- default $namespace = "urn:".$serviceName;
	...
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		...
		if(false == $soapaction) {
			if (isset($_SERVER)) {
				...
				$SCRIPT_NAME = str_replace("?".$_SERVER['QUERY_STRING'],"",$_SERVER['REQUEST_URI']);
				$HTTPS = isset($_SERVER['HTTPS']) ? $_SERVER['HTTPS'] : (isset($HTTP_SERVER_VARS['HTTPS']) ? $HTTP_SERVER_VARS['HTTPS'] : 'off');
			} 
			elseif (isset($HTTP_SERVER_VARS)) {
				$SERVER_NAME = $HTTP_SERVER_VARS['SERVER_NAME'];
				//$SCRIPT_NAME = isset($HTTP_SERVER_VARS['PHP_SELF']) ? $HTTP_SERVER_VARS['PHP_SELF'] : $HTTP_SERVER_VARS['SCRIPT_NAME'];
				$SCRIPT_NAME = str_replace("?".$HTTP_SERVER_VARS['QUERY_STRING'],"",$HTTP_SERVER_VARS['REQUEST_URI']);
...
class wsdl extends nusoap_base {
    ...
    */
    function webDescription(){
		...
		if (isset($_SERVER)) {
			$PHP_SELF = str_replace("?".$_SERVER['QUERY_STRING'],"",$_SERVER['REQUEST_URI']);//$_SERVER['PHP_SELF'];
		} elseif (isset($HTTP_SERVER_VARS)) {
			$PHP_SELF = str_replace("?".$HTTP_SERVER_VARS['QUERY_STRING'],"",$HTTP_SERVER_VARS['REQUEST_URI']);//$HTTP_SERVER_VARS['PHP_SELF'];
		} else {
			$this->setError("Neither _SERVER nor HTTP_SERVER_VARS is available");
		}
