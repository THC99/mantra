...
class XML2Array {
    ...
    public static function &createArray($input_xml) {
		...
		if(is_string($input_xml)) {
			...
			if(!$parsed) {
                //file_put_contents(TMPDIR."xml2array.txt",'Error parsing the XML string');
				throw new Exception('[XML2Array] Error parsing the XML string.');
			}
		} else {
			if(get_class($input_xml) != 'DOMDocument') {
                //file_put_contents(TMPDIR."xml2array.txt",'The input XML object should be of type: DOMDocument');
...
function tidyXML($data){
	...
	$tidy=new tidy;
	$tidy->parseString($data,$tidyconf,'utf8'); // Rapihkan susunan XML
	$tidy->cleanRepair();
